...
def run_multi_samples(data_type):
    ...
    # plot_df = copy.deepcopy(summary_df[['dataset_name', 'model_id', 'model_type', 'num_samples', 'accuracy_training', 'accuracy_validation']])
    # plot_df.groupby(['num_samples','model_id']).mean().unstack().plot(ax=ax)
    # fig.savefig('Learning_curve_{}.png'.format(data_type))
...
def run_multi_cross_validation_samples(data_type, n_folds=2):
    ...
    # plot_df = copy.deepcopy(summary_df[['dataset_name', 'model_id', 'model_type', 'num_samples', 'accuracy_training', 'accuracy_validation']])
    # plot_df.groupby(['num_samples','model_id']).mean().unstack().plot(ax=ax)
    # fig.savefig('Learning_curve_{}_wXvalidation.png'.format(data_type))
